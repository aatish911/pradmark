...
if (isset($_POST['query'])) {
 ...
 $univer = date("ymdHis");
 $ae = $conn->Prepare('INSERT INTO newser (main,univer,idblog,idname,seoname,editor,userid,user,userpic,title,helper,brief,address,shortdesc,image,longdesc) VALUES (' . $conn->qstr
  ($main) . ',' . $conn->qstr($univer) . ',' . $conn->qstr($idblog) . ',' . $conn->qstr($idname) . ',' . $conn->qstr($seoname) . ',' . $conn->qstr($editor) . ',' . $conn->qstr($kori) .
